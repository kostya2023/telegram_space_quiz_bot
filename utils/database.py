        ...
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS Questions (
                    ...
                    image_path TEXT,  -- Путь к изображению (опционально)
                    hint TEXT,  -- Подсказка к вопросу (опционально)
                    description TEXT  -- Описание правильного ответа (опционально)
                )
            ''')
...
def add_question(question_text: str, option1: str, option2: str, option3: str, option4: str, correct_option: int, image_path: Optional[str] = None, hint: Optional[str] = None, description: Optional[str] = None) -> bool:
    """
    Добавляет вопрос в базу данных с вариантами ответов, изображением, подсказкой и описанием правильного ответа.
    ...
    :param hint: Подсказка к вопросу (опционально).
    :param description: Описание правильного ответа (опционально).
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            # Добавляем вопрос с вариантами ответов, изображением, подсказкой и описанием
            cursor.execute('''
                INSERT INTO Questions (question_text, option1, option2, option3, option4, correct_option, image_path, hint, description)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (question_text, option1, option2, option3, option4, correct_option, image_path, hint, description))
...
def get_question(question_id: int) -> Optional[Dict[str, Any]]:
    """
    Возвращает вопрос с вариантами ответов, изображением, подсказкой и описанием правильного ответа.

    :param question_id: ID вопроса.
    :return: Словарь с вопросом, вариантами ответов, изображением, подсказкой и описанием, или None, если вопрос не найден.
    """
    try:
        # Подключаемся к базе данных
        with sqlite3.connect(db_path) as conn:
            ...
            # Получаем вопрос
            cursor.execute('''
                SELECT question_text, option1, option2, option3, option4, correct_option, image_path, hint, description
                FROM Questions
                WHERE question_id = ?
            ''', (question_id,))
            ...
            # Формируем словарь с вопросом, вариантами ответов, изображением, подсказкой и описанием
            return {
                "question_text": question[0],
                "options": [question[1], question[2], question[3], question[4]],
                "correct_option": question[5],
                "image_path": question[6],  # Путь к изображению
                "hint": question[7],  # Подсказка
                "description": question[8]  # Описание правильного ответа
            }
...
def update_question_hint(question_id: int, hint: str) -> bool:
    """
    Обновляет подсказку для вопроса.

    :param question_id: ID вопроса.
    :param hint: Новая подсказка.
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            # Фиксируем изменения в базе данных
            conn.commit()
            print(f"Подсказка для вопроса {question_id} успешно обновлена.")
            return True  # Успешно

    except sqlite3.Error as e:
        # Обработка ошибок при обновлении подсказки
        print(f"Ошибка при обновлении подсказки для вопроса {question_id}: {e}")
        return False  # Произошла ошибка
...
if __name__ == "__main__":
    ...
    add_question(
        ...
        "Эта планета известна своими полосами и большим красным пятном.",  # Подсказка
        "Юпитер является самой большой планетой Солнечной системы, его масса в 2.5 раза больше массы всех остальных планет вместе взятых."  # Описание
    )
    ...
    add_question(
        ...
        2,  # Млечный Путь
        "./imgs/question2.jpg",
        "Эта галактика имеет форму спирали и содержит миллиарды звёзд.",  # Подсказка
        "Млечный Путь — это галактика, в которой находится наша Солнечная система. Она содержит около 200 миллиардов звёзд."  # Описание
    )
    ...
    add_question(
        ...
        3,  # 2 (Фобос и Деймос)
        "./imgs/question3.jpg",
        "Эти спутники имеют неправильную форму и названы в честь мифологических персонажей.",  # Подсказка
        "У Марса два спутника: Фобос и Деймос. Они были открыты в 1877 году и имеют неправильную форму."  # Описание
    )
    ...
    add_question(
        ...
        2,  # Чёрная дыра
        "./imgs/question4.jpg",
        "Этот объект обладает настолько сильной гравитацией, что даже свет не может его покинуть.",  # Подсказка
        "Чёрная дыра — это область пространства-времени с настолько сильной гравитацией, что ничто, даже свет, не может её покинуть."  # Описание
    )
    ...
    add_question(
        ...
        2,  # 108 минут (Гагарин)
        "./imgs/question5.jpg",
        "Этот полёт совершил советский космонавт в 1961 году.",  # Подсказка
        "Первый полёт человека в космос длился 108 минут. Его совершил Юрий Гагарин 12 апреля 1961 года на корабле 'Восток-1'."  # Описание
    )
    ...
    add_question(
        ...
        2,  # Метан
        "./imgs/question6.jpg",
        "Этот газ также присутствует в атмосфере Земли и является одним из основных компонентов природного газа.",  # Подсказка
        "Метан в атмосфере Нептуна поглощает красный свет и отражает синий, что придаёт планете её характерный цвет."  # Описание
    )
    ...
    add_question(
        ...
        2,  # Олимп (Марс)
        "./imgs/question7.jpg",
        "Этот вулкан находится на Марсе и в три раза выше Эвереста.",  # Подсказка
        "Олимп — это крупнейший вулкан Солнечной системы, расположенный на Марсе. Его высота составляет около 21,9 км."  # Описание
    )
    ...
    add_question(
        ...
        2,  # Сириус
        "./imgs/question8.jpg",
        "Эта звезда находится в созвездии Большого Пса и является одной из ближайших к Земле.",  # Подсказка
        "Сириус — самая яркая звезда на ночном небе. Она находится в созвездии Большого Пса и удалена от Земли на 8,6 световых лет."  # Описание
    )
    ...
    add_question(
        ...
        "./imgs/question9.jpg",
        "Год на Венере длится меньше, чем её собственные сутки.",  # Подсказка
        "Год на Венере длится 225 земных дней, что меньше, чем один день на Венере, который длится 243 земных дня."  # Описание
    )
    ...
    add_question(
        ...
        1,  # Вояджер-1
        "./imgs/question10.jpg",
        "Этот аппарат был запущен в 1977 году и продолжает передавать данные из межзвёздного пространства.",  # Подсказка
        "Вояджер-1 — первый космический аппарат, который покинул Солнечную систему. Он был запущен в 1977 году и до сих пор передаёт данные."  # Описание
    )
    ...
    add_question(
        ...
        1,  # Звездопад
        "./imgs/question11.jpg",
        "Это явление часто называют 'падающими звёздами', хотя к звёздам оно не имеет отношения.",  # Подсказка
        "При падении метеорита в атмосферу возникает явление, называемое 'звездопадом'. Это светящийся след, который оставляет метеорит, сгорая в атмосфере."  # Описание
    )
    ...
    add_question(
        ...
        4,  # Дальняя сторона
        "./imgs/question12.jpg",
        "Эта сторона Луны никогда не видна с Земли из-за приливного захвата.",  # Подсказка
        "Обратная сторона Луны называется 'дальняя сторона'. Она никогда не видна с Земли из-за приливного захвата, который синхронизирует вращение Луны с её орбитой."  # Описание
    )
    ...
    add_question(
        ...
        4,  # Водород (≈73%)
        "./imgs/question13.jpg",
        "Этот элемент является самым лёгким и самым распространённым во Вселенной.",  # Подсказка
        "Водород составляет около 73% массы Солнца. Он является основным элементом, участвующим в термоядерных реакциях в ядре Солнца."  # Описание
    )
    ...
    add_question(
        ...
        "./imgs/question14.jpg",
        "Кольца Сатурна состоят из миллиардов частиц льда и пыли.",  # Подсказка
        "Кольца Сатурна состоят из тысяч тонких колец, которые образованы миллиардами частиц льда и пыли."  # Описание
    )
    ...
    add_question(
        ...
        1,  # Уран
        "./imgs/question15.jpg",
        "Эта планета имеет ось вращения, наклонённую почти на 98 градусов.",  # Подсказка
        "Уран вращается 'лёжа на боку', так как его ось вращения наклонена почти на 98 градусов относительно плоскости орбиты."  # Описание
    )
    ...
    add_question(
        ...
        2,  # Силиконы
        "./imgs/question16.jpg",
        "Эта форма жизни предполагается как альтернатива углеродной основе.",  # Подсказка
        "Силиконы — это гипотетическая форма жизни, основанная на кремнии, который может заменять углерод в биохимических процессах."  # Описание
    )
    ...
    add_question(
        ...
        3,  # Кеплер
        "./imgs/question17.jpg",
        "Этот телескоп был запущен специально для поиска планет за пределами Солнечной системы.",  # Подсказка
        "Телескоп Кеплер обнаружил множество экзопланет, находящихся в 'зоне жизни', где условия могут быть пригодны для существования жидкой воды."  # Описание
    )
    ...
    add_question(
        ...
        1,  # Андромеда
        "./imgs/question18.jpg",
        "Эта галактика является ближайшей крупной галактикой к Млечному Пути.",  # Подсказка
        "Туманность Андромеды — это ближайшая крупная галактика, которая через 4 миллиарда лет столкнётся с Млечным Путём, образуя новую галактику."  # Описание
    )
    ...
    add_question(
        ...
        "./imgs/question19.jpg",
        "Из-за медленного вращения вокруг своей оси день на Меркурии длится дольше, чем его год.",  # Подсказка
        "Один день на Меркурии длится 176 земных дней из-за резонанса вращения, вызванного гравитационным влиянием Солнца."  # Описание
    )
    ...
    add_question(
        ...
        3,  # Точка Лагранжа
        "./imgs/question20.jpg",
        "В этой точке гравитационные силы двух тел уравновешиваются.",  # Подсказка
        "Точка Лагранжа — это точка в системе двух тел, где гравитационные силы уравновешиваются, и объект может оставаться в устойчивом положении."  # Описание
    )
    ...
    add_question(
        ...
        1,  # Железо
        "./imgs/question21.jpg",
        "Этот элемент является конечным продуктом термоядерных реакций в звёздах.",  # Подсказка
        "Железо образуется в ядрах умирающих звёзд в результате термоядерных реакций. Оно является конечным продуктом этих реакций."  # Описание
    )
    ...
    add_question(
        ...
        3,  # Расстояния
        "./imgs/question22.jpg",
        "Один парсек равен примерно 3,26 световых года.",  # Подсказка
        "Парсек — это единица измерения космических расстояний. Один парсек равен примерно 3,26 световых года."  # Описание
    )
    ...
    add_question(
        ...
        "./imgs/question23.jpg",
        "Этот тип сверхновой возникает в двойных системах, где один из компонентов — белый карлик.",  # Подсказка
        "Взрыв сверхновой типа Ia происходит в двойных системах, где белый карлик перетягивает вещество со звезды-компаньона, пока не достигнет критической массы и не взорвётся."  # Описание
    )
    ...
    add_question(
        ...
        "./imgs/question24.jpg",
        "Большая часть Вселенной состоит из тёмной материя и тёмной энергии.",  # Подсказка
        "Видимая материя составляет всего около 5% Вселенной. Остальное — это тёмная материя (27%) и тёмная энергия (68%)."  # Описание
    )
    ...
    add_question(
        ...
        1,  # Пульсар
        "./imgs/question25.jpg",
        "Этот объект является быстро вращающейся нейтронной звездой.",  # Подсказка
        "Пульсары — это быстро вращающиеся нейтронные звёзды, которые испускают регулярные радиоимпульсы."  # Описание
    )

    print("База данных успешно инициализирована с 25 вопросами, изображениями, подсказками и описаниями!")